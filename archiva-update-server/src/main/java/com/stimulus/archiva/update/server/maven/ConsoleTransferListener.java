...
final class ConsoleTransferListener
    ...
    public void transferInitiated(TransferEvent event) {
        final String message = event.getRequestType() == TransferEvent.RequestType.PUT
                ? "Uploading"
                : "Downloading";
    ...
    private String getStatus(final long complete, final long total) {
        if (1024 <= total)
            return kb(complete) + " / " + kb(total) + " KB ";
        else if (0 <= total)
            return complete + " / " + total + " B ";
        else if (1024 <= complete)
            return kb(complete) + " KB ";
        else
            return complete + " B ";
    }
    ...
    public void transferSucceeded(TransferEvent event) {
        ...
        if (contentLength >= 0) {
            ...
            String len = contentLength >= 1024 ? kb(contentLength) + " KB" : contentLength + " B";
    ...
    protected long kb(long bytes) { return (bytes + 1023) / 1024; }
}
